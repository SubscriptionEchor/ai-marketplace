...
const TOTAL_ITEMS = 100; // Example total count

export function MarketplaceView() {
  const [currentPage, setCurrentPage] = useState(1);
  
  const totalPages = Math.ceil(TOTAL_ITEMS / ITEMS_PER_PAGE);
  const items = Array.from({ length: ITEMS_PER_PAGE }, (_, i) => i + 1 + (currentPage - 1) * ITEMS_PER_PAGE)
    .filter(item => item <= TOTAL_ITEMS);
  ...
  return (
    <div className="px-4 sm:px-0">
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((item) => (
          <motion.div
            ...
            transition={{ duration: 0.3, delay: Math.min((item % ITEMS_PER_PAGE) * 0.02, 0.5) }}
      ...
      {/* Pagination */}
      <div className="mt-8 pb-8 flex items-center justify-between border-t border-gray-200 pt-6">
        <div className="flex items-center">
